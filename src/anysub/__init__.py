...
__version__ = importlib.metadata.version("anysub")
...
async def async_run(
    ...
    **kwargs: T.Any,
    ...
    Documentation of the all the supported values is provided in the
    [StdLib docs](https://docs.python.org/3/library/asyncio-eventloop.html#asyncio.loop.subprocess_exec).
            ...
            async with proc:
                async with anyio.create_task_group() as tg:
                    if proc.stdout is not None:
                        ...
                        tg.start_soon(handle_stream, proc_stdout, stdout_buffer, sys.stdout, echo and echo_stdout)

                    if proc.stderr is not None:
                        proc_stderr = anyio.streams.text.TextReceiveStream(transport_stream=proc.stderr)
                        tg.start_soon(handle_stream, proc_stderr, stderr_buffer, sys.stderr, echo and echo_stderr)
